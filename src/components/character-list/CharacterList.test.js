...
describe('CharacterList Component',  () => {
    it('Should Load Data', async () => {
        axios.get.mockReturnValue(new Promise(resolve => resolve(characters)));
    })
    it('Should list and search for a char when input change search field', async ()=> {
        const { getByTestId, getByText, queryByText } = renderComponent()
        const searchName = 'Abe Sapien'

        expect(getByText('loading...')).toBeInTheDocument();
        expect(queryByText(searchName)).not.toBeInTheDocument();

        const searchFieldNode = await waitForElement(
        ...
        expect(searchFieldNode.value).toEqual(searchName)
